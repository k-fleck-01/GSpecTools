...
class Spectrum:
    ...
    def __init__(self, xbins: np.ndarray, yvals: np.ndarray, yerrs: np.ndarray) -> None:
        ...
        nbins = nedges - 1
        if yvals.size != nbins:
            raise RuntimeError("Number of bins does not match number of specified values or errors.")
    ...
    def visualise(self, ax: plt.Axes, show_errs: bool = False, 
                  lbl: str = "", fmt: Optional[str] = None) -> None:
        ...
        # Plot errorbars
        if show_errs:
            ax.errorbar(xcentres, yvals, yerrs, fmt = c+'.', markersize = .5)
    ...
    def scale_to_match(self, other: 'Spectrum') -> None:
        ...
        if y_data.size!= y_true.size:
            raise RuntimeError("Spectrum does not have the same number of bins as the other spectrum.")
    ...
    def convolve(self, func: Callable, fano: Optional[float] = None) -> 'Spectrum':
        """Convolution of the Spectrum object with some function. Mathematically, this
        calculates
